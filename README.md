...
**Analysis** — Claude Sonnet receives the amount mismatches and the list of already-covered accounts (so it does not re-raise reconciled items), then generates open-ended attention points using the [rule set](rules/aandachtspunten.md). The missing/notFilledIn categories are not sent to the LLM.
