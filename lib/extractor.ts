  ...
  "accounts": [
    {
      ...
      "amounts": {
        ...
        "mortgage": { "interestPaid": 8400, "remainingDebt": 215000 }
      }
    }
  ],
  "metadata": {
...
- Extract account numbers and identifiers exactly as they appear in the document — do not mask, redact, or abbreviate them (e.g. write "johndoe" not "******ist")
...
- accountNumber is the IBAN associated with that entry, or null if none is shown. Dutch IBANs are always 18 characters (NL + 2 digits + 4-letter bank code + 10 digits = NL##BBBB##########). IBANs are sometimes split across two lines in the PDF — the amount for that entry will appear on the continuation line, after the final digits of the IBAN. Reconstruct the full 18-character IBAN and associate the amount from the continuation line with it (e.g. "ING Betaalrekening NL22 INGB 0673 / 3457 85   € 5.140" → accountNumber "NL22 INGB 0673 3457 85", amount 5140). If the field label embeds a non-IBAN account identifier (e.g. "Bankrekening: ING Creditcardrekening 2100 3093 2649"), extract that identifier into accountNumber as-is, including any spaces — the analyzer normalises whitespace when matching. Mortgage entries may use a "Nummer" prefix instead of the standard "Aftrekbare rente van schuld" pattern — for example "Nummer 1926.58.069 / Betaalde rente in 2025: €105" must be extracted as a box 1 entry with accountNumber "Nummer1926.58.069" and amount -105 (negative, as it is a deduction)
