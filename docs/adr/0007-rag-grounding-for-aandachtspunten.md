...
- **Chunking** (`chunker.ts`): heading-aware paragraph packing, target ~1000 chars (soft; heading + overlap may exceed it) per chunk, ~120 chars of overlap with the preceding chunk of the same section. A heading always stays in the chunk with the text it introduces, and a chunk that opens a new section carries no overlap from the previous one (#93). Pure and dependency-free.
