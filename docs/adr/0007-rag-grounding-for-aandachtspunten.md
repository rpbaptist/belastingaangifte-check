...
- **Chunking** (`chunker.ts`): heading-aware paragraph packing, a target of ~1000 chars per chunk (soft: a chunk's heading run plus overlap can take it past the target), ~120 chars of overlap with the preceding chunk of the same section. A heading always stays in the chunk with the text it introduces, and a chunk that opens a new section carries no overlap from the previous one (#93). Pure and dependency-free.
